...
ORYX.Core.Canvas = ORYX.Core.AbstractShape.extend({
	...
	construct: function(options, stencil, facade) {
		...
		*/
		...
		this.node.setAttributeNS(null, 'stroke', 'red');
		...
		this.rootNode.oncontextmenu = function() {return false;};
			...
			//高亮当前节点
			if(ORYX.Utils.getParamFromUrl("linkId")){
				setTimeout(function(){
					for(svgg of document.getElementsByClassName("children")[0].children){
						if(svgg.id == "svg-"+ORYX.Utils.getParamFromUrl("linkId")){
							for(rect of svgg.children[0].children[0].children[0].children){
								if(rect.id.indexOf("bg_frame") > 0){
...
ORYX.Editor = {
	...
	*/
	...
	_handleMouseDown: function(event, uiObj) {
		...
		var nodename = "";
		var incoming = [];
		if(elementController.properties && !elementController.properties["oryx-conditionsequenceflow"]
			&& elementController.isResizable && elementController.isSelectable){
			nodename = elementController.properties["oryx-name"];
			if(elementController.incoming){
				for(var i in elementController.incoming){
					var elObj = elementController.incoming[i];
					if(elObj.properties && elObj.properties["oryx-conditionsequenceflow"]){
						incoming.push(elObj.properties["oryx-conditionsequenceflow"].split("'")[1]);
					}
				}
			}
		}
		var endaction = [];
		var outgoingaction = [];
		if(elementController.outgoing){
			for(var i in elementController.outgoing){
				var tmp = elementController.outgoing[i];
				if(tmp.properties){
					outgoingaction.push({id:tmp.properties["oryx-conditionsequenceflow"].split("'")[1],name:tmp.properties["oryx-name"]});
				}
				if(tmp.outgoing){
					for(var j in tmp.outgoing){
						var tmp1 = tmp.outgoing[j];
						if(tmp1.outgoing && tmp1.outgoing.length == 0){endaction.push(tmp.properties["oryx-conditionsequenceflow"].split("'")[1]);break;}
						if(tmp1.outgoing && !tmp1.isResizable){
							for(let wg of tmp1.outgoing){
								if(wg.outgoing){
									for(let wg1 of wg.outgoing){
										if(wg1.outgoing && wg1.outgoing.length == 0){endaction.push(wg.properties["oryx-conditionsequenceflow"].split("'")[1]);break;}
									}
								}
							}
						}
		...
		var msg = {linkId:elementController.resourceId,name:nodename,incomingAction:incoming.join(","),endAction:endaction.join(","),outgoingAction:outgoingaction};
		console.log(msg);
