  ...
  <div v-loading.fullscreen.lock="loading" class="app-container queryList">
    ...
    >
      ...
      <template slot="table">
        ...
        <el-table-column label="应收保函费用(元)" prop="amount" width="180">
          <template slot-scope="scope">
            {{ scope.row.amount / 100 }}
          </template>
        </el-table-column>
        <el-table-column label="实收保函费用(元)" prop="amount" width="180">
          <template slot-scope="scope">
            {{ scope.row.amount / 100 }}
          </template>
        </el-table-column>
        <el-table-column label="付款成功次数" prop="paymentNo" width="220" />
        <el-table-column label="应退费用" prop="paymentNo" width="220" />
        <el-table-column label="客户名称" prop="paymentNo" width="220" />
        <el-table-column label="统一信用代码" prop="paymentNo" width="220" />
        ...
        <el-table-column
          ...
          width="180"
        />
        <el-table-column label="审核状态" prop="status" width="180">
          <template slot-scope="scope">
            {{
              $selectDictLabel(
                $store.getters.dictData.dzbh_zfzt || [],
        ...
        <el-table-column label="支付流水号" prop="paymentDate" width="180" />

        <el-table-column label="备注" prop="payerAccountNumber" width="180" />
      </template>
      <template slot="other">
        <!--  自定义组件部分  -->
      </template>
...
export default {
  ...
  computed: {
    formData: function(params) {
      return [
        { label: '开始日期', model: 'paymentStartDate', type: 'date' },
        { label: '结束日期', model: 'paymentEndDate', type: 'date' },
        { label: '业务流水号', model: 'applyno' },
        { label: '保函业务流水号', model: 'paymentNo' },
        { label: '客户名称', model: 'paymentNo' },
        { label: '统一信用代码', model: 'paymentNo' },
        {
          label: '退款状态',
          model: 'status',
          type: 'select'
